...
interface ConfirmRetrieveModalProps {
  open: boolean;
  title: string;
  subtitle: string;
  onCancel: () => void;
...
export function ConfirmRetrieveModal({
  open,
  title,
  subtitle,
  onCancel,
  ...
  return (
    <Dialog open={open} onOpenChange={onCancel}>
      <CustomDialogContent className='!max-w-[95%] sm:!max-w-[519px] !rounded-[20px] py-[30px] px-[25px] flex flex-col items-center text-center bg-[var(--card-background)] !gap-0 border-[var(--border-dark)]'>
        <div className='flex items-center justify-center w-16 md:w-[100px] h-16 md:h-[100px] rounded-full bg-greenaccent-100 bg-opacity-10 mb-6 mx-auto'>
          <Refresh size='40' color='var(--secondary)' />
        </div>
        ...
        <div className='flex gap-2 md:gap-4 w-full justify-center'>
          ...
          <Button
            onClick={onRetrieve}
            type='submit'
            className='h-[48px] px-6 md:px-12 bg-[var(--secondary)] hover:bg-[var(--secondary)] rounded-full font-semibold text-white'
          >
