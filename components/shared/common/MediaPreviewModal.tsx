...
const MediaThumbnail = ({
  ...
  return (
    <div
      className={`w-full h-20 bg-gray-100 rounded-lg overflow-hidden cursor-pointer hover:opacity-80 transition-opacity flex items-center justify-center ${className}`}
    >
      {isVideo ? (
...
export default function MediaPreviewModal({
  ...
  useEffect(() => {
    try {
      ...
      console.error('Error initializing MediaPreviewModal:', error);
      setIsClient(false);
    }
  }, []);
      ...
      <DialogContent className='lg:max-w-[940px] lg:w-[940px] max-w-[90%] [&>button]:hidden max-h-[90vh] p-0 overflow-hidden bg-[var(--card-background)] rounded-none'>
        ...
        <div className='pb-6 lg:max-w-[940px] lg:w-[940px] max-w-full w-full'>
          ...
          <div className='relative mb-6 max-w-full w-full px-8'>
            <div className='max-w-full w-full aspect-[16/10] overflow-hidden'>
              {isClient && (
                <Swiper
              ...
              <button className='main-swiper-button-prev h-8 w-8 sm:h-10 sm:w-10 absolute left-0 top-1/2 transform -translate-y-1/2 bg-[var(--primary)] rounded-full flex items-center justify-center shadow-md z-10 hover:opacity-90 transition-opacity'>
                <ArrowLeft2 size={20} color='white' />
              </button>
              <button className='main-swiper-button-next absolute right-0 top-1/2 transform -translate-y-1/2 sm:w-10 sm:h-10 w-8 h-8 bg-[var(--primary)] rounded-full flex items-center justify-center shadow-md z-10 hover:opacity-90 transition-opacity'>
