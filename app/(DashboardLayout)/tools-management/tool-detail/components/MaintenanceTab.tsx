...
import { TOOL_DETAIL_MAINTENANCE_COLUMNS } from '@/constants/tablecolumns';
...
export const MaintenanceTab: React.FC<MaintenanceTabProps> = ({
  ...
  const transformData = (apiData: ToolItemDetail[]) => {
    return apiData.map(item => {
      ...
      return {
        ...
        barcode: barcode || '-',
        returnedBy: {
          name: returnedByName || 'Available',
          avatar: avatarUrl,
          id: returnedById,
        },
        employeeType: designation || '',
        assignedJob: assignedJobName || '-',
  ...
  if (loading) {
    return <TableSkeleton columns={5} rows={6} showActions={true} />;
  }

  if (transformedData.length === 0) {
    return (
      <NoDataFound
        title='No Maintenance Tools'
        description='No tools under maintenance found.'
      />
    );
  }
  ...
  return (
    <DynamicTable
      ...
      columns={TOOL_DETAIL_MAINTENANCE_COLUMNS}
      actions={maintenanceTableActions}
      emptyMessage='No maintenance tools found'
