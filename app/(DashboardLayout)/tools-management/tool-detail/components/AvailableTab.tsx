...
import { TOOL_DETAIL_AVAILABLE_COLUMNS } from '@/constants/tablecolumns';
...
export const AvailableTab: React.FC<AvailableTabProps> = ({
  ...
  const transformData = (apiData: ToolItemDetail[]) => {
    return apiData.map(item => {
      ...
      const { name: assignedJobName } = assignedJob || {};
      ...
      return {
        ...
        employeeType: designation || 'Ready',
  ...
  if (loading) {
    return <TableSkeleton columns={6} rows={6} showActions={true} />;
  }
  ...
  if (transformedData.length === 0) {
    return (
      <NoDataFound
        ...
        description='No available tools found.'
      />
    );
  }
  ...
  return (
    <DynamicTable
      ...
      columns={TOOL_DETAIL_AVAILABLE_COLUMNS}
