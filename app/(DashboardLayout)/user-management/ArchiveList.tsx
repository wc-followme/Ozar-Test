				...
				<>
					{users.length === 0 && !loading ? (
						<div className='h-full md:h-[calc(100vh_-_220px)] w-full'>
							<NoDataFound title={noDataTitle} description={noDataDescription} buttonText='' showButton={false} />
						</div>
					) : (
						<div className='grid grid-cols-autofit xl:grid-cols-autofit-xl gap-4 sm:gap-3 xl:gap-6'>
							{users.map(
								({ uuid, name, role, phone_number, email, profile_picture_url, status, id }) => (
									<UserCard
										key={uuid}
										name={name}
										role={role?.name || ''}
										...
										image={
											profile_picture_url
												? (process.env['NEXT_PUBLIC_CDN_URL'] || '') + profile_picture_url
											: ''
										}
										status={status === CommonStatus.ACTIVE}
										onToggle={() => onToggle(id, status === CommonStatus.ACTIVE)}
